...
use soroban_sdk::{
    ...
    Address, Env, IntoVal, String, Symbol, MuxedAddress,
};
...
fn test() {
        ...
        std::vec![(
            ...
            AuthorizedInvocation {
                function: AuthorizedFunction::Contract((
                    ...
                    (&user1, &user2, 500_i128, 200_u32).into_val(&e),
                )),
                sub_invocations: std::vec![]
            }
        )]
    ...
    // Test transfer
    token.transfer(&user1, &MuxedAddress::from(user2.clone()), &600);
    assert_eq!(
        e.auths(),
        std::vec![(
            user1.clone(),
            AuthorizedInvocation {
                function: AuthorizedFunction::Contract((
                    ...
                    (&user1, &MuxedAddress::from(user2.clone()), 600_i128).into_val(&e),
                )),
                sub_invocations: std::vec![]
            }
        )]
    ...
    // Test transfer_from
    token.transfer_from(&user2, &user1, &user3, &400);
    assert_eq!(
        e.auths(),
        std::vec![(
            user2.clone(),
            AuthorizedInvocation {
                function: AuthorizedFunction::Contract((
                    ...
                    (&user2, &user1, &user3, 400_i128).into_val(&e),
                )),
                sub_invocations: std::vec![]
            }
        )]
    ...
    assert_eq!(token.balance(&user3), 400);
}
...
fn test_constructor_mint() {
    ...
    let token_contract = e.register(
        ...
        (
            ...
            String::from_str(&e, "name"),
            String::from_str(&e, "symbol"),
            5000_i128, // initial_mint = 5000
        ),
    );
    let token = TokenClient::new(&e, &token_contract);

    assert_eq!(token.balance(&admin), 5000);
}
...
fn transfer_insufficient_balance() {
    ...
    token.transfer(&user1, &MuxedAddress::from(user2.clone()), &1001);
}
